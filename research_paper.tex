...
Single-Image EDSR (Bicubic Input) & 27.00 & 0.7437 \\
Dual-Image EDSR (Bicubic+Gaussian, Channel Fusion) & \textbf{27.75} & \textbf{0.7803} \\
\bottomrule
\end{tabular}
\end{table}

As shown in Table~\ref{tab:nwpu_results}, the proposed dual-image EDSR with bicubic-centric inputs outperforms the single-image baseline in PSNR and SSIM, demonstrating that fusing complementary observations consistently improves reconstruction fidelity over a single degraded input. The dual-image model achieves a gain of $+0.74$~dB PSNR and $+0.037$ SSIM over Single-Image EDSR, and a gain of $+1.47$~dB PSNR and $+0.090$ SSIM over bicubic interpolation.
...
The experimental results demonstrate that the proposed dual-image super-resolution framework consistently improves reconstruction quality over single-image baselines under both supervised and reference-free settings. The proposed method achieves a PSNR gain of $+0.74$~dB and a SSIM gain of $+0.037$ over the single-image baseline on NWPU-RESISC45, demonstrating that fusing complementary bicubic-centric observations enhances reconstruction fidelity. Furthermore, the blind evaluation results on Sentinel-2 imagery suggest that channel-fusion reconstruction provides stronger visual fidelity and generalization under practical satellite imaging conditions.
...
Experimental results on the NWPU-RESISC45 dataset demonstrated a PSNR of 27.75~dB and SSIM of 0.780 for the proposed dual-image method, compared to 27.00~dB PSNR and 0.744 SSIM for the single-image EDSR baseline, representing gains of $+0.74$~dB and $+0.037$ respectively. These findings suggest that the proposed approach provides a practical and reliable solution for enhancing spatial resolution in remote sensing applications where high-resolution ground truth is unavailable.
