...
\begin{abstract}
% 150--200 words
The lack of high-resolution ground truth limits the practical application of remote sensing image super-resolution, which is crucial for improving spatial details in Earth observation data. In order to take advantage of complementary information from two low-resolution observations of the same scene, this paper suggests a dual-image super-resolution framework based on a modified Enhanced Deep Super-Resolution Network (EDSR). In order to capture realistic sensor variation in satellite imagery, the suggested method uses a bicubic-centric dual input strategy, where one input is obtained via bicubic downsampling and the second via Gaussian-blur followed by bicubic downsampling. Shallow features from both inputs are concatenated in a channel-fusion architecture, which then reconstructs the fused representation using a shared residual body.Experiments on the NWPU-RESISC45 remote sensing dataset with a 70:15:15 train,validation,test split demonstrate consistent improvement over single-image EDSR baselines, with PSNR increasing from 27.73~dB to 29.05~dB (+1.32~dB) and SSIM from 0.773 to 0.821 (+0.049). The suggested framework's practical applicability is further confirmed by blind evaluation on actual Sentinel-2 imagery.
\end{abstract}
